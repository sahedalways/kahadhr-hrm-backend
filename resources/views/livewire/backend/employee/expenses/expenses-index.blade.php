                            ...
                            <tbody>
                                ...
                                @forelse($infos as $row)
                                    <tr>
                                        ...
                                        <td>{{ $row->submitted_at ? date('d M, Y', strtotime($row->submitted_at)) : 'N/A' }}
                                        </td>

                                        <td>
                                            @php
                                                $submittedAt = \Carbon\Carbon::parse($row->submitted_at);
                                                $canEditOrDelete = $submittedAt->addHours(24)->isFuture();
                                            @endphp


                                            {{-- Edit Button --}}
                                            <a href="#"
                                                class="badge {{ $canEditOrDelete ? 'bg-info text-white' : 'bg-secondary text-light disabled' }}"
                                                style="{{ $canEditOrDelete ? '' : 'pointer-events: none;' }}"
                                                @if ($canEditOrDelete) wire:click="editExpense({{ $row->id }})"
       data-bs-toggle="modal"
       data-bs-target="#editExpenseModal" @endif>
                                                Edit
                                            </a>

                                            {{-- Delete Button --}}
                                            <a href="#"
                                                class="badge {{ $canEditOrDelete ? 'bg-danger text-white' : 'bg-secondary text-light disabled' }}"
                                                style="{{ $canEditOrDelete ? '' : 'pointer-events: none;' }}"
                                                @if ($canEditOrDelete) wire:click.prevent="$dispatch('confirmDelete', {{ $row->id }})" @endif>
                                                Delete
                                            </a>
                                        </td>
